...
export const BoostedRefund: FC<BoostedRefundParams> = ({amount, selectedAddress, onSuccess, onFail}) => {
    ...
    return <FlexBlock $direction="column" style={{flex: 1}}>
        ...
        <ul style={{textAlign: 'left'}}>
            ...
            <li>Insert your address (<code style={{ cursor: "pointer" }} onClick={() => navigator.clipboard.writeText(selectedAddress)}>{truncateAddress(selectedAddress)}</code><span onClick={() => navigator.clipboard.writeText(selectedAddress)} style={{fontSize: '18px', cursor: "pointer"}}>&#x2398;</span>)</li>
